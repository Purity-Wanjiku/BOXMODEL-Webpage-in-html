...
$(document).ready(function () {
	// WHY NICE - Slider
	if ($(".js-slide-why").length) {
		let $slideWhy = $(".js-slide-why"),
			$slideBtn1 = $(".js-slide-why-btn-1"),
			$slideBtn2 = $(".js-slide-why-btn-2"),
			$slideBtn3 = $(".js-slide-why-btn-3"),
			$slide1 = $(".js-slide-why-1"),
			$slide2 = $(".js-slide-why-2"),
			$slide3 = $(".js-slide-why-3"),
			$slide1Arrow = $(".js-slide-why-btn-1-arrow"),
			$slide2Arrow = $(".js-slide-why-btn-2-arrow"),
			$slide3Arrow = $(".js-slide-why-btn-3-arrow");

		$slideBtn1.on("click", function (e) {
			e.preventDefault();

			$slideWhy.addClass("c-slide--1");
			$slideWhy.removeClass("c-slide--2 c-slide--3");

			$slideBtn1.addClass("is-active");
			$slideBtn2.removeClass("is-active");
			$slideBtn3.removeClass("is-active");

			$slide1.removeClass("is-off");
			$slide2.addClass("is-off");
			$slide3.addClass("is-off");

			$slide2Arrow.removeClass("hidden");
			$slide1Arrow.addClass("hidden");
			$slide3Arrow.addClass("hidden");
		});

		$slideBtn2.on("click", function (e) {
			e.preventDefault();

			$slideWhy.addClass("c-slide--2");
			$slideWhy.removeClass("c-slide--1 c-slide--3");

			$slideBtn1.removeClass("is-active");
			$slideBtn2.addClass("is-active");
			$slideBtn3.removeClass("is-active");

			$slide2.removeClass("is-off");
			$slide1.addClass("is-off");
			$slide3.addClass("is-off");

			$slide3Arrow.removeClass("hidden");
			$slide1Arrow.addClass("hidden");
			$slide2Arrow.addClass("hidden");
		});

		$slideBtn3.on("click", function (e) {
			e.preventDefault();

			$slideWhy.addClass("c-slide--3");
			$slideWhy.removeClass("c-slide--1 c-slide--2");

			$slideBtn1.removeClass("is-active");
			$slideBtn2.removeClass("is-active");
			$slideBtn3.addClass("is-active");

			$slide3.removeClass("is-off");
			$slide1.addClass("is-off");
			$slide2.addClass("is-off");

			$slide1Arrow.removeClass("hidden");
			$slide2Arrow.addClass("hidden");
			$slide3Arrow.addClass("hidden");
		});
	}
	...
	if ($("#works-nav").length) {
		const $weWantIn = $("#we-want-in"),
			$weWantInBtn = $("#we-want-in-btn"),
			$gettingSetUp = $("#getting-set-up"),
			$gettingSetUpBtn = $("#getting-set-up-btn"),
			$timeToGrow = $("#time-to-grow"),
			$timeToGrowBtn = $("#time-to-grow-btn"),
			$worksNav = $("#works-nav"),
			$pageContainer = $("#PageContainer"),
			$howItWorks = $("#how-it-works");

		// 1.0 = Find top position of nav
		let $worksNavTop = $worksNav.offset().top;

		// 1.1 = Locate We Want In section
		let $weWantInTop = $weWantIn.offset().top;

		// 1.2 = Locate Getting Set Up section
		let $getSetUpTop = $gettingSetUp.offset().top;

		// 1.3 = Locate Time to Grow section
		let $timeToGrowTop = $timeToGrow.offset().top;
		// g('time grow ' + $timeToGrowTop);

		$(window).scroll(function (e) {
			var scroll = $(window).scrollTop();

			// 1.0 = Works nav fixed on page
			if (scroll > $worksNavTop - 1) {
				$worksNav.addClass("c-works-nav--fixed");
				$pageContainer.addClass("no-transform");
				$howItWorks.addClass("pt-24");
			} else {
				$worksNav.removeClass("c-works-nav--fixed");
				$pageContainer.removeClass("no-transform");
				$howItWorks.removeClass("pt-24");
			}

			// 2.0 = We Want
			if (scroll > $weWantInTop - 1 && scroll < $getSetUpTop - 1) {
				$weWantInBtn.addClass("is-active");
			} else {
				$weWantInBtn.removeClass("is-active");
			}

			// 3.0 = Getting Set Up
			if (scroll > $getSetUpTop - 1 && scroll < $timeToGrowTop - 1) {
				$gettingSetUpBtn.addClass("is-active");
			} else {
				$gettingSetUpBtn.removeClass("is-active");
			}

			// 4.0 = Time to Grow
			if (scroll > $timeToGrowTop - 1) {
				$timeToGrowBtn.addClass("is-active");
			} else {
				$timeToGrowBtn.removeClass("is-active");
			}
		});
	}

	// LP Scroll
	$(".js-scroll-el").on("click", function (e) {
		e.preventDefault();
		e.stopPropagation();
		var self = $(this);
